...
public class ChatWebSocketHandler extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		if (userSessions.containsKey(dto.getSender_no())) {
			...
			ChatMsg entity = ChatMsg.builder().sendMember(member).chatRoom(chatRoom).msgContent(dto.getMsg_content()).build();
	...
	/*
	 * author : 조은성 history : 20205--3-24 param : url. key data return : value data
	 * role : WebSocketSession url parsing
	 */
	// getQueryParam(session,"sonderNo"); -> 3
